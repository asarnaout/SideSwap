...
- **The app shell is nearly untested.** `launcher.test.tsx` has 6 tests and covers none of the driving HUD — fuel drain, refuel pricing, the 8s fine debounce, the gig double-credit guard, the cutscene trigger/effect wiring, minimap pins, music mute. Changing any of that is invisible to `npm test` (the cutscene *choreography* is covered by `tests/cutsceneScript.test.ts`; the app wiring is not).
